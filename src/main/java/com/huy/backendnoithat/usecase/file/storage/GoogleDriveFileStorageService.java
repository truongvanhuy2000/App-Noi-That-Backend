package com.huy.backendnoithat.usecase.file.storage;
...
import com.huy.backendnoithat.usecase.data.DataProcessingService;
...
public class GoogleDriveFileStorageService implements FileStorageService {
    ...
    public void updateFileInfo(int fileID, FileType fileType, SavedFileDTO savedFileDTO) {
        log.info("Update file info fileID {} fileType {} : {}", fileID, fileType, savedFileDTO);
    ...
    public void deleteFile(int fileID, FileType fileType) {
        log.info("Delete file fileID {} fileType {}", fileID, fileType);
    ...
    public List<SavedFileDTO> getAllFileOfUser(FileType fileType) {
        log.info("Get all file of user fileType {}", fileType);
    ...
    @Override
    public List<SavedFileDTO> getAllFileOfUserById(int userID) {
        log.info("Get all file of user by ID {}", userID);

        List<SavedFileEntity> entities = fileStorageDAO.findAllByAccountId(userID);
        return entities.stream().map(savedFileEntityDTOMapper::toDTO).toList();
    }

    @Override
    public SavedFileDTO getFileInfo(int fileID, FileType fileType) {
        log.info("Get file info fileID {} fileType {}", fileID, fileType);
    ...
    public SavedFileDTO getFile(int fileID, FileType fileType, OutputStream outputStream) {
        log.info("Get file fileID {} fileType {}", fileID, fileType);
    ...
    public void updateFile(UploadFile uploadFile, FileType fileType, int fileID) {
        log.info("Update file fileID {} fileType {}", fileID, fileType);
    ...
    public SavedFileDTO saveFile(FileType fileType, UploadFile uploadFile) {
        log.info("Save file fileType {}", fileType);
    ...
    public PaginationResponse<List<SavedFileDTO>> find(
        ...
        log.info("Find file paginationRequest {} fileSearchRequest {}", paginationRequest, fileSearchRequest);
